  ...
  int bsz, int seqlen, int nhead, int head_dim)
{
  ...
  int h = tid % (head_dim / 2); tid /= (head_dim / 2);
  int n = tid % nhead; tid /= nhead;
  int s = tid % seqlen; tid /= seqlen;
  int b = tid % bsz; tid /= bsz;
  if (b >= bsz) return;
  int input_idx = ((b * seqlen + s) * nhead + n) * head_dim + h * 2;
  ...
  int bsz, int seqlen, int nhead, int head_dim)
{
    auto stream = at::cuda::getCurrentCUDAStream().stream();
    int tbsize = 128;
    int nblocks = (bsz * seqlen * nhead * (head_dim / 2) + tbsize - 1) / tbsize;
    const dim3 threads(tbsize);
    const dim3 blocks(nblocks);
    cuApplyRotaryEmb<<<blocks, threads, 0, stream>>>(
      input, freqs, output, bsz, seqlen, nhead, head_dim);
}
...
void cuda_rotary_emb(
    ...
    at::Tensor* output) 
{
  int bsz = input->sizes()[0];
  int seqlen = input->sizes()[1];
  int nhead = input->sizes()[2];
  int head_dim = input->sizes()[3];
  using namespace at;
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT_INOUT_TYPES(
